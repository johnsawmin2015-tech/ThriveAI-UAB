...
- `GEMINI_API_KEY` — optional Google AI Studio key. When set, the API uses the
  official Gemini SDK.
- `OPENAI_API_KEY` — optional OpenAI key used when no Gemini key is present.
  Defaults to `gpt-4o-mini` via `OPENAI_MODEL`.
- `AI_GATEWAY_API_KEY` — optional Vercel AI Gateway key. Production OIDC is not
  used unless this key is set, because Gateway currently requires a credit card.
- `AI_MODEL` — optional Gemini model override; defaults to `gemini-3.7-flash`.
